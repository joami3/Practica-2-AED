...
library(ggplot2)
weather<- load("./data/weatherAUS.RData")
...
grafico2
grafico3<-ggplot(ds,aes(x=MinTemp,y=MaxTemp,colour = RainTomorrow))+geom_point()+ggtitle("Temperaturas")
grafico3
...
graf1<- ggplot(ds,aes(x=Temp9am,y=Pressure9am,shape=RainToday))+geom_point()
graf1
graf2<- ggplot(ds,aes(x=Temp9am,y=Pressure9am,colour =RainToday))+geom_point() + labs(title="Temperatura y Presión a las 9AM",subtitle = "Comparacion entre las temperaturas y la presiona las 9AM",x="Temperatura",y="Presion",color="Dias lluviosos")
graf2
#Se aprecia mejor con el color 
...
iris.grafico1
iris.grafico2 <- ggplot(iris.tidy, aes(x = Species, y = Value, colour = Part)) +geom_jitter() +facet_grid(.~Measure)
iris.grafico2
#Con geom_jitter se aprecia mejor la cantidad de puntos   
...
g2
g2<-ggplot(DatosEjer4,aes(x=Fecha,y=Ventas,colour = Fabricante,linetype = Fabricante))+geom_line()+geom_point()
g2
g2<-ggplot(DatosEjer4,aes(x=Fecha,y=Ventas,colour = Fabricante,linetype = Fabricante))+geom_line()+geom_point()+labs(title="Ventas mensuales de diferentes compañías tecnológicas")
g2
...
str("Experimento.Rdata")

g1<- ggplot(Experimento, aes(x = Dosis, y = Altura,color = Complemento,linetype = Complemento,shape = Complemento))+geom_point()+geom_line()+theme_minimal() +labs(title = "Altura según Dosis y Complemento",x = "Dosis",y = "Altura")
g1
g2 <- g1 +geom_errorbar(aes(ymin = Altura - Error_Altura,ymax =Altura + Error_Altura),width = 0.2)
g2

g3 <- ggplot(Experimento, aes(x = Dosis, y = Altura,color = Complemento)) +geom_point() +geom_line() +geom_errorbar(aes(ymin = Altura - Error_Altura,ymax = Altura + Error_Altura),width = 0.2) +scale_color_brewer(palette = "Reds") +theme_minimal() +labs(title = "Altura con barras de error (paleta roja)",x = "Dosis", y = "Altura")
g3
...
caudal<-ggplot(ice.river.df)+geom_line(aes(x=time,y=flow.vat,colour = "Jokulsa Eystri"))+geom_line(aes(x=time,y=flow.jok,colour ="Vatnsdalsa"))+scale_colour_manual(values = c("Vatnsdalsa" = "blue", "Jokulsa Eystri" = "red"))+labs(title="Caudales de los rios islandeses Vatnsdalsa (azul) y Jokulsa Eystri(azul)",y="Caudal",x="Años")
caudal
...
library(ggplot2)
load("./data/weatherAUS_distributions.Rdata")#no me carga este archivo 

g1 <- ggplot(weatherAUS_distributions, aes(x = factor(Year), y = MinTemp)) +geom_boxplot(fill = "skyblue", color = "black") +labs(title = "Distribución de temperaturas mínimas por año",x = "Año", y = "Temperatura mínima (°C)")
...
library(ggplot2)
load("./data/iceRiver.RData")
ice.river.df %>% pivot_longer(names_to= "River",values_to = "Flow",cols=flow.vat:flow.jok)%>%ggplot(aes(x=time,y=Flow))+geom_line(aes(colour = River))+geom_point(size=0.01)
